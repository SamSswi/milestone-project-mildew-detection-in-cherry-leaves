...
def leaves_visualizer_body():
    ...
    if difference_avr_var:
        avg_var_healthy = plt.imread(f'outputs/{version}/avg_and_var_image_healthy.png')
        avg_var_powdery_mildew = plt.imread(f'outputs/{version}/avg_and_var_image_powdery_mildew.png')
        st.warning('We notice the average and the image variability images reveal subtle patterns where we could intuitively differentiate healthy cherry leaves from cherry leaves with powdery mildew. The healthy leaves exhibit a hightened intensity of green color while the leaves infected with powdery mildew displays a multitude of pale speckles on the leaf surface. The pattern can be observed when comparing the image variability images of the two labels.')
        st.image(avg_var_healthy, caption='Average and Variability Image - Healthy Leaves')
        st.image(avg_var_powdery_mildew, caption='Average and Variability Image - Powdery Mildew Infected Leaves')
        st.write('---')

    difference_avr_labels = st.checkbox('Difference between the average image of healthy leaves and powdery mildew infected leaves')
    if difference_avr_labels:
        difference_labels = plt.imread(f'outputs/{version}/difference_image.png')
        st.warning("We notice this study didn't show patterns where we could intuitively differentiate between the average images of the two labels.")
        st.image(difference_labels, caption='Difference between the average images')
        st.write('---')
